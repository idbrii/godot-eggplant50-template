...
func get_input() -> Dictionary:
	var move := Input.get_vector("move_left", "move_right", "move_up", "move_up")
	return {
		"x": move.x,
		"y": move.y,
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("action2"):
		self.scale.y = 0.5
	elif Input.is_action_just_released("action2"):
		self.scale.y = 2
	jump_logic(delta)
	apply_gravity(delta)
...
func jump_logic(_delta: float) -> void:
	...
	if jump_coyote_timer > 0 and jump_buffer_timer > 0 and not is_jumping:
		...
		$AudioStreamPlayer.position = self.position
...
#	print("player")
#	print(body)
#	if body.has_method("send_player_hit"):
#		body.send_player_hit()
	if $RayCast2D.is_colliding():
#		print("Player Died! Raycast is colliding")
		pass
	else:
		pass
#		print("no")
